...
- The **verifiable counterpart** of this PRD is
  [`acceptance-criteria.md`](./acceptance-criteria.md). Every requirement
  described here maps to one or more acceptance criteria there, and
  every AC is exercised by a test (or explicitly tagged as manual
  `/verify` for voice / Realtime). See `CLAUDE.md` → "SDD workflow" for
...
- Decisions are recorded in §3 (cross-wave) and §4.3 / §5 / §6 (per
  ...
  in the same PR that introduces the change, alongside the
  corresponding AC updates.
