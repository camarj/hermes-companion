...
description: Reviews changes to backend/realtime.py and the surrounding voice/audio code for known correctness pitfalls. Use after modifying the Realtime WebSocket proxy or vision frame injection. Runs in a fresh context so it isn't biased by the implementing session.
...
## 5. DB writes inside the recv loop
...
## 6. The `companion.*` protocol is symmetric

If you rename or add a custom event in `backend/realtime.py`, the matching handler must exist in the React port (`frontend/src/hooks/useRealtime.ts`). The protocol is small and easy to grep — verify both sides agree.
